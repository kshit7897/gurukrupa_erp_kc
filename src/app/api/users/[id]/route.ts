...
export async function PUT(request: NextRequest, context: any) {
  try {
    ...
    const { name, role, password, email, permissions } = body;
    if (typeof name === 'string') user.name = name;
    if (typeof role === 'string') user.role = role;
    if (typeof email === 'string') user.email = email;
    if (Array.isArray(permissions)) user.permissions = permissions.map((p: any) => String(p));
    if (password) {
      const hashed = await bcrypt.hash(password, 10);
      user.password = hashed;
    }
    await user.save();
    const safe = { id: (user as any)._id.toString(), username: user.username, email: (user as any).email || null, name: user.name, role: user.role, permissions: user.permissions || [] };
