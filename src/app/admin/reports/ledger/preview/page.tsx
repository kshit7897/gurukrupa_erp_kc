        ...
        <div className="relative transition-transform print:transform-none print:w-full" style={{ width: '210mm' }}>
          <div id="ledger-content" className="bg-white shadow-xl print:shadow-none min-h-[297mm] text-slate-900 print:w-full print:m-0 p-6">
            <div className="flex justify-between items-center border-b mb-4 pb-4">
              <div>
                <h1 className="text-lg font-bold text-slate-900">{party?.name || 'Party Ledger'}</h1>
                {party && <p className="text-sm text-slate-600">{party.address || ''} • {party.mobile || ''}</p>}
              </div>
              <div className="text-right text-sm text-slate-600">{fromDate || ''} - {toDate || ''}</div>
            </div>
            ...
            {!loading && transactions.length > 0 && (
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-slate-800 text-white"><th className="text-left py-2 px-3">Date</th><th className="text-left py-2 px-3">Ref</th><th className="text-left py-2 px-3">Type</th><th className="text-right py-2 px-3">Debit</th><th className="text-right py-2 px-3">Credit</th><th className="text-right py-2 px-3">Balance</th></tr>
                </thead>
