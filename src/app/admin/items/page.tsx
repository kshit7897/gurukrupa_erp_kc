...
export default function Items() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal isOpen={isDeleteConfirmOpen} onClose={() => { setIsDeleteConfirmOpen(false); setDeleteTarget(null); }} title="Confirm delete" footer={<><Button variant="ghost" onClick={() => { setIsDeleteConfirmOpen(false); setDeleteTarget(null); }}>Cancel</Button><Button onClick={async () => { if (!deleteTarget) return; try { await api.items.delete(deleteTarget); await loadItems(); setIsDeleteConfirmOpen(false); setDeleteTarget(null); setNotification({ type: 'success', message: 'Product deleted' }); } catch (err: any) { const errorMessage = err?.message || 'Failed to delete product'; setNotification({ type: 'error', message: errorMessage }); } }}>Delete</Button></>}> <div className="py-4">Are you sure you want to delete this product? This will remove it from the database.</div></Modal>
