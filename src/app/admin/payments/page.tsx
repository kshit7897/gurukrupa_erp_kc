...
export default function PaymentsPage() {
 ...
 'use client';
import React, { useEffect, useState } from 'react';
import { Card, Input, Select, Button, Table, SoftLoader } from '../../../components/ui/Common';
import { api } from '../../../lib/api';
...
export default function PaymentsPage() {
  ...
  const [form, setForm] = useState({ partyId: '', amount: '', date: new Date().toISOString().split('T')[0], mode: 'cash', reference: '', notes: '' });
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [pList, payList] = await Promise.all([api.parties.list(), api.payments.list()]);
      setParties(pList || []);
      setPayments(payList || []);
    } catch (err) {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const payload = { partyId: form.partyId, amount: Number(form.amount), date: form.date, mode: form.mode, reference: form.reference, notes: form.notes };
      const created = await api.payments.add(payload);
      setPayments(prev => [created, ...prev]);
      setForm({ partyId: '', amount: '', date: new Date().toISOString().split('T')[0], mode: 'cash', reference: '', notes: '' });
    } catch (err: any) {
  ...
  const visiblePayments = filterParty ? payments.filter(p => p.partyId === filterParty) : payments;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card title="Record Payment">
          <form onSubmit={handleSubmit} className="space-y-4">
            <Select label="Party" options={[{ label: 'Select party', value: '' }, ...parties.map(p => ({ label: p.name, value: p.id }))]} value={form.partyId} onChange={(e) => handleChange('partyId', e.target.value)} />
            <Input label="Amount" type="number" value={form.amount} onChange={(e) => handleChange('amount', e.target.value)} />
            <Input label="Date" type="date" value={form.date} onChange={(e) => handleChange('date', e.target.value)} />
            <Select label="Mode" options={[{ label: 'Cash', value: 'cash' }, { label: 'Online', value: 'online' }, { label: 'Cheque', value: 'cheque' }]} value={form.mode} onChange={(e) => handleChange('mode', e.target.value)} />
            <Input label="Reference" value={form.reference} onChange={(e) => handleChange('reference', e.target.value)} />
        ...
        <div className="md:col-span-2 space-y-4">
          <Card title="Payments" className="">
            <div className="mb-4 flex items-center gap-3">
              <Select label="Filter by Party" options={[{ label: 'All Parties', value: '' }, ...parties.map(p => ({ label: p.name, value: p.id }))]} value={filterParty} onChange={(e) => setFilterParty(e.target.value)} />
            </div>

            {loading ? (
              <div className="py-8 flex items-center justify-center"><SoftLoader text="Loading payments..." /></div>
            ) : (
              <Table headers={[ 'Date', 'Party', 'Amount', 'Mode', 'Reference' ]}>
                {visiblePayments.map((p) => (
                  <tr key={p.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3">{p.date}</td>
                    <td className="px-4 py-3">{(parties.find(x => x.id === p.partyId) || { name: p.partyId }).name}</td>
                    <td className="px-4 py-3">₹ {Number(p.amount).toFixed(2)}</td>
                    <td className="px-4 py-3">{p.mode}</td>
                    <td className="px-4 py-3">{p.reference || '-'}</td>
                  </tr>
                ))}
                {visiblePayments.length === 0 && <tr><td colSpan={5} className="px-4 py-6 text-center text-slate-500">No payments found</td></tr>}
              </Table>
            )}
          </Card>
        </div>
