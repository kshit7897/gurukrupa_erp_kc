    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="md:hidden space-y-4">
        {loading && items.length === 0 ? (
          ...
          <div className="text-center py-8 text-slate-500">No transactions found in this period</div>
        ) : items.map((item) => {
            ...
            <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="text-right">
                  <p className="text-lg font-black text-slate-800">₹ {item.amount.toLocaleString()}</p>
                </div>
              </div>
              ...
              {(item.referenceNo || item.note) && (
                <p className="text-xs text-slate-500 mb-3">
                  {item.referenceNo && <span className="font-bold text-slate-700 mr-2 text-[10px] uppercase bg-slate-100 px-1 rounded">{item.referenceNo}</span>}
                  {item.note || '-'}
                </p>
              )}
              ...
              <div className="flex gap-2 pt-3 border-t border-slate-100">
                <button 
                  ...
                  className="flex-1 py-2 bg-white border border-slate-200 rounded-md text-blue-600 font-semibold text-sm"
                >
                  Edit
                </button>
                <button 
                  onClick={() => handleDelete(item.id)} 
                  className="flex-1 py-2 bg-red-600 text-white rounded-md font-semibold text-sm"
                >
                  Delete
                </button>
              </div>
      ...
      {/* Desktop table */}
      <Card className="p-0 overflow-hidden rounded-2xl border-slate-200/60 shadow-md hidden md:block">
