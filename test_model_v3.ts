import dbConnect from './src/lib/mongodb';
import OtherTxn from './src/lib/models/OtherTxn';
import fs from 'fs';

async function diagnose() {
  await dbConnect();
  
  const testData = {
    companyId: 'TEST_CO',
    txnType: 'TRANSFER',
    date: '2026-01-31',
    amount: 123.45,
    fromId: 'PID_FROM',
    fromName: 'Name From',
    toId: 'PID_TO',
    toName: 'Name To'
  };

  const test = await OtherTxn.create(testData);
  const fetched = await OtherTxn.findById(test._id).lean();
  
  const results = {
    txnType: { value: fetched.txnType, ok: fetched.txnType === 'TRANSFER' },
    fromId: { value: fetched.fromId, ok: fetched.fromId === 'PID_FROM' },
    toId: { value: fetched.toId, ok: fetched.toId === 'PID_TO' }
  };

  fs.writeFileSync('test_results.json', JSON.stringify(results, null, 2));
  console.log('Results saved to test_results.json');

  await OtherTxn.findByIdAndDelete(test._id);
  process.exit(0);
}

diagnose().catch(console.error);
